...
#ROOTFS=$1
ROOTFS="out/target/product/q201_9377/ramdisk.img"
...
if [ "$2" != "m" ]; then
#    make -C common O=../$KERNEL_OUT q201.dtd ARCH=arm64 CROSS_COMPILE=$PREFIX_CROSS_COMPILE
    make -C common O=../$KERNEL_OUT gxm_q201_1g.dtb ARCH=arm64 CROSS_COMPILE=$PREFIX_CROSS_COMPILE
    make -C common O=../$KERNEL_OUT gxm_q201_2g.dtb ARCH=arm64 CROSS_COMPILE=$PREFIX_CROSS_COMPILE
fi
...
if [ "$2" != "m" ]; then
    out/host/linux-x86/bin/mkbootimg --kernel common/../$KERNEL_OUT/arch/arm64/boot/Image \
        --ramdisk ${ROOTFS} \
        --second common/../$KERNEL_OUT/arch/arm64/boot/dts/amlogic/gxm_q201_1g.dtb \
        --output ./out/target/product/q201_9377/boot.img
    ls -l ./out/target/product/q201_9377/boot.img
    echo "boot.img done"
fi
